// ============================================================
//  RawaGameScene.js — Gameplay Habitat Rawa (Self-Contained)
//  Progres disimpan: localStorage key "rawa_maxLevel"
// ============================================================

class RawaGameScene extends Phaser.Scene {
    constructor() {
        super("RawaGameScene");
    }
    ...
    static get ANIMALS() {
        return {
            correct: ["rawa_buaya", "rawa_bangau", "rawa_ikangabus", "rawa_katak", "rawa_ularair","sungai_katak","sungai_buaya"],
            trap: [
                ...
                "sungai_bangau", "sungai_belut", "sungai_kepiting",
    ...
    preload() {
        if (!this.textures.exists("rawa_bg"))        this.load.image("rawa_bg",        "Asset/Rawa/bgRawa.png");
        if (!this.textures.exists("rawa_timer"))     this.load.image("rawa_timer",     "Asset/Rawa/Timer.png");
        if (!this.textures.exists("rawa_keranjang")) this.load.image("rawa_keranjang", "Asset/Rawa/keranjang.png");
        if (!this.textures.exists("rawa_awan"))      this.load.image("rawa_awan",      "Asset/Rawa/Awan.png");
        ...
        if (!this.cache.audio.exists("bgmRawa"))       this.load.audio("bgmRawa",       "Asset/Sound/bgmRawa.mp3");
    }
    ...
    create() {
        ...
        if (this.cache.audio.exists("bgmRawa")) {
            this.bgmRawa = this.sound.add("bgmRawa", { loop: true, volume: 0.4 });
            if (!this.isMuted) this.bgmRawa.play();
        }

        this.cameras.main.fadeIn(400, 0, 0, 0);

        const cfg = RawaGameScene.LEVEL_CONFIG[this.level];
        this.scoreCorrect = 0;
        this.scoreWrong   = 0;
    ...
    update() {
        ...
        if (this.hudCorrect) this.hudCorrect.setText("✅ " + this.scoreCorrect + " / " + RawaGameScene.LEVEL_CONFIG[this.level].target);
        if (this.hudWrong)   this.hudWrong.setText("❌ " + this.scoreWrong);
    }

    _buildBackground(w, h) {
        if (this.textures.exists("rawa_bg")) {
            this.add.image(w / 2, h / 2, "rawa_bg").setDisplaySize(w, h);
        } else {
            this.add.rectangle(w / 2, h / 2, w, h, 0x0a1a0a);
        }
        this.add.rectangle(w / 2, h / 2, w, h, 0x020800, 0.22);

        const ui = Math.min(w / 900, h / 550);
        if (this.textures.exists("rawa_awan")) {
            const sc = ui * 0.6;
            const c1 = this.add.image(-160, h * 0.08, "rawa_awan").setScale(sc).setAlpha(0.45).setDepth(2).setTint(0xb8d4a0);
            const c2 = this.add.image(w + 160, h * 0.17, "rawa_awan").setScale(sc * 0.72).setAlpha(0.30).setDepth(2).setTint(0x90b878);
            this.tweens.add({ targets: c1, x: w + 180, duration: 36000, ease: "Linear", repeat: -1, onRepeat: () => { c1.x = -160; } });
            this.tweens.add({ targets: c2, x: -160,    duration: 50000, ease: "Linear", repeat: -1, onRepeat: () => { c2.x = w + 180; } });
        }

        this.add.text(12, h - 12, "🌾 RAWA  |  Level " + this.level, {
            fontSize: "15px", fontFamily: "Arial, sans-serif",
            color: "#ffffff", stroke: "#051208", strokeThickness: 3,
        }).setOrigin(0, 1).setAlpha(0.75).setDepth(3);
    }

    _buildBasket(w, h) {
        const ui = Math.min(w / 900, h / 550);
        this.basket = this.physics.add.image(w / 2, h - 72 * ui, "rawa_keranjang");
    ...
    _buildHUD(w, h) {
        const ui    = Math.min(w / 900, h / 550);
        const style = { fontSize: `${Math.floor(18 * ui)}px`, fontFamily: "Arial Black, Arial, sans-serif", color: "#ffffff", stroke: "#051208", strokeThickness: 4 };
        this.hudCorrect = this.add.text(12, 12, "✅ 0 / " + RawaGameScene.LEVEL_CONFIG[this.level].target, style).setOrigin(0, 0).setDepth(10);
        this.hudWrong   = this.add.text(12, 12 + 25 * ui, "❌ 0", style).setOrigin(0, 0).setDepth(10);

        this.timerBg = this.add.image(w - 15 * ui, 15 * ui, "rawa_timer").setOrigin(1, 0).setDepth(9);
    ...
    spawnAnimal(w) {
        ...
        const pool      = isCorrect ? RawaGameScene.ANIMALS.correct : RawaGameScene.ANIMALS.trap;
        ...
        animal.setScale(((RawaGameScene.SIZE_MAP[texKey] || 90) * ui / maxDim) * Phaser.Math.FloatBetween(0.88, 1.12));
    ...
    _onCatch(basket, animal) {
        if (animal.getData("isCorrect")) {
            this.scoreCorrect++;
            this._showFeedback(animal.x, animal.y, "+1", "#94d870");
    ...
    _endLevel() {
        ...
        if (this.bgmRawa) this.bgmRawa.stop();
        this.animalGroup.getChildren().forEach(a => a.destroy());
        this.animalGroup.clear(true, true);

        const isWin = this.scoreCorrect >= RawaGameScene.LEVEL_CONFIG[this.level].target;
        if (isWin) {
            const saved = parseInt(localStorage.getItem("rawa_maxLevel") || "1");
            localStorage.setItem("rawa_maxLevel", String(Math.min(Math.max(saved, this.level + 1), 3)));
    ...
    _showResult(isWin) {
        ...
        const overlay = this.add.rectangle(w / 2, h / 2, w, h, 0x020800, 0.72).setDepth(30).setAlpha(0);
        ...
        let stars = 0;
        if (isWin) { stars = 1; if (this.scoreWrong <= 3) stars = 2; if (this.scoreWrong === 0) stars = 3; if (this.scoreCorrect >= RawaGameScene.LEVEL_CONFIG[this.level].target + 5) stars = 3; }
        ...
        this.add.text(pcX, pcY - pH * 0.56, isWin ? "LUAR BIASA!" : "AYO COBA LAGI!", { fontSize: `${Math.floor(pH * 0.08)}px`, fontFamily: "Arial Black, Arial, sans-serif", color: isWin ? "#94d870" : "#ff8080", stroke: "#000", strokeThickness: 5 }).setOrigin(0.5).setDepth(32).setAlpha(0);
        ...
        const btnDefs = [
            { key: "ui_home",  action: () => { this.cameras.main.fadeOut(300, 0, 0, 0); this.cameras.main.once("camerafadeoutcomplete", () => { this.scene.start("RawaScene"); }); } },
            { key: "ui_retry", action: () => { this.cameras.main.fadeOut(300, 0, 0, 0); this.cameras.main.once("camerafadeoutcomplete", () => { this.scene.start("RawaGameScene", { level: this.level }); }); } },
        ];
        if (isWin && this.level < 3) btnDefs.push({ key: "ui_next", action: () => { this.cameras.main.fadeOut(300, 0, 0, 0); this.cameras.main.once("camerafadeoutcomplete", () => { this.scene.start("RawaGameScene", { level: this.level + 1 }); }); } });
...
window.RawaGameScene = RawaGameScene;
