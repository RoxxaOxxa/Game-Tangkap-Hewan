// ============================================================
//  SavanaScene.js — Pilih Level Savana (dengan Level Lock)
//  Progres disimpan: localStorage key "savana_maxLevel"
// ============================================================

class SavanaScene extends Phaser.Scene {
    constructor() {
        super("SavanaScene");
        this.isMuted = false;
    }
    ...
    preload() {
        this.load.image("savana_bg",     "Asset/Savana/bgSavana.png");
        this.load.image("savana_lv1",    "Asset/Savana/1.png");
        this.load.image("savana_lv2",    "Asset/Savana/2.png");
        this.load.image("savana_lv3",    "Asset/Savana/3.png");
        this.load.image("savana_gembok", "Asset/Savana/gembok.png");
        this.load.image("savana_awan",   "Asset/Savana/Awan.png");
    ...
    _getMaxLevel() {
        return parseInt(localStorage.getItem("savana_maxLevel") || "1");
    }
    ...
    _buildLayout() {
        ...
        this.add.rectangle(w / 2, h / 2, w, h, 0x1a0e00, 0.35);

        // ── Awan animasi — hangat kuning kecoklatan ──
        const cloudSc = ui * 0.6;
        const cloud1 = this.add.image(-160, h * 0.10, "savana_awan").setScale(cloudSc).setAlpha(0.65).setDepth(1).setTint(0xffd890);
        const cloud2 = this.add.image(w + 160, h * 0.19, "savana_awan").setScale(cloudSc * 0.75).setAlpha(0.45).setDepth(1).setTint(0xffc060);

        this.tweens.add({ targets: cloud1, x: w + 180, duration: 32000, ease: "Linear", repeat: -1, onRepeat: () => { cloud1.x = -160; } });
        this.tweens.add({ targets: cloud2, x: -160,    duration: 46000, ease: "Linear", repeat: -1, onRepeat: () => { cloud2.x = w + 180; } });

        // ── Judul ──
        const title = this.add.text(w / 2, -60, "🦁  HABITAT SAVANA", {
            fontSize: `${Math.floor(48 * ui)}px`,
            fontFamily: "Arial Black, Arial, sans-serif",
            color: "#ffd080", stroke: "#1a0e00", strokeThickness: 6,
        ...
        const sub = this.add.text(w / 2, -60, "Pilih Level", {
            ...
            color: "#ffb848", stroke: "#1a0e00", strokeThickness: 3,
        }).setOrigin(0.5).setAlpha(0).setDepth(2);
        ...
        // ── Tombol Level ──
        const levels = [
            { key: "savana_lv1", level: 1, desc: "Mudah" },
            { key: "savana_lv2", level: 2, desc: "Sedang" },
            { key: "savana_lv3", level: 3, desc: "Sulit" },
        ];
        ...
        levels.forEach((lvl, i) => {
            ...
            if (!unlocked) btn.setTint(0x604530);
                    ...
                    if (unlocked) {
                        this._makeBouncyButton(btn, btnScale, () => {
                            ...
                            this.cameras.main.once("camerafadeoutcomplete", () => {
                                this.scene.start("SavanaGameScene", { level: lvl.level });
                            });
                        });
                    } else {
            ...
            if (!unlocked) {
                const lock = this.add.image(bx, btnY, "savana_gembok").setOrigin(0.5).setAlpha(0).setDepth(4);
        ...
        backTxt.on("pointerout",  () => { this.tweens.add({ targets: backTxt, scale: 1,    duration: 120, ease: "Back.out(2)" }); backTxt.setStyle({ color: "#ffb848" }); });
...
window.SavanaScene = SavanaScene;
