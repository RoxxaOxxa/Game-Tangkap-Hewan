// ============================================================
//  DanauScene.js — Pilih Level Danau (dengan Level Lock)
//  Progres disimpan: localStorage key "danau_maxLevel"
// ============================================================

class DanauScene extends Phaser.Scene {
    constructor() {
        super("DanauScene");
        this.isMuted = false;
    }
    ...
    preload() {
        this.load.image("danau_bg",     "Asset/Danau/bgDanau.png");
        this.load.image("danau_lv1",    "Asset/Danau/1.png");
        this.load.image("danau_lv2",    "Asset/Danau/2.png");
        this.load.image("danau_lv3",    "Asset/Danau/3.png");
        this.load.image("danau_gembok", "Asset/Danau/gembok.png");
        this.load.image("danau_awan",   "Asset/Danau/Awan.png");
    ...
    _getMaxLevel() {
        return parseInt(localStorage.getItem("danau_maxLevel") || "1");
    }
    ...
    _buildLayout() {
        ...
        this.add.rectangle(w / 2, h / 2, w, h, 0x001428, 0.38);

        // ── Awan animasi ──
        const cloudSc = ui * 0.6;
        const cloud1 = this.add.image(-160, h * 0.10, "danau_awan").setScale(cloudSc).setAlpha(0.70).setDepth(1);
        const cloud2 = this.add.image(w + 160, h * 0.19, "danau_awan").setScale(cloudSc * 0.75).setAlpha(0.50).setDepth(1);
        ...
        // ── Judul ──
        const title = this.add.text(w / 2, -60, "🏞️  HABITAT DANAU", {
            fontSize: `${Math.floor(48 * ui)}px`,
            fontFamily: "Arial Black, Arial, sans-serif",
            color: "#a0e8ff", stroke: "#001a33", strokeThickness: 6,
        ...
        const sub = this.add.text(w / 2, -60, "Pilih Level", {
            ...
            color: "#70ccff", stroke: "#001428", strokeThickness: 3,
        }).setOrigin(0.5).setAlpha(0).setDepth(2);
        ...
        // ── Tombol Level ──
        const levels = [
            { key: "danau_lv1", level: 1, desc: "Mudah" },
            { key: "danau_lv2", level: 2, desc: "Sedang" },
            { key: "danau_lv3", level: 3, desc: "Sulit" },
        ];
        ...
        levels.forEach((lvl, i) => {
            ...
            if (!unlocked) btn.setTint(0x405060);
                    ...
                    if (unlocked) {
                        this._makeBouncyButton(btn, btnScale, () => {
                            ...
                            this.cameras.main.once("camerafadeoutcomplete", () => {
                                this.scene.start("DanauGameScene", { level: lvl.level });
                            });
                        });
                    } else {
            ...
            if (!unlocked) {
                const lock = this.add.image(bx, btnY, "danau_gembok").setOrigin(0.5).setAlpha(0).setDepth(4);
            ...
            const descTxt = this.add.text(bx, btnY + 80 * ui, lvl.desc, {
                ...
                fontFamily: "Arial, sans-serif",
                color: unlocked ? "#70ccff" : "#405060",
                stroke: "#001428", strokeThickness: 3,
        ...
        backTxt.on("pointerout",  () => { this.tweens.add({ targets: backTxt, scale: 1,    duration: 120, ease: "Back.out(2)" }); backTxt.setStyle({ color: "#70ccff" }); });
