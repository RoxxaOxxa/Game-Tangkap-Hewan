// ============================================================
//  LautGameScene.js — Gameplay Habitat Laut (Self-Contained)
//  Progres disimpan: localStorage key "laut_maxLevel"
// ============================================================

class LautGameScene extends Phaser.Scene {
    constructor() {
        super("LautGameScene");
    }
    ...
    static get ANIMALS() {
        return {
            correct: ["laut_hiu", "laut_lumbalumba", "laut_orca", "laut_uburubur", "laut_penyu", "laut_kepiting"],
            trap: [
    ...
    preload() {
        if (!this.textures.exists("laut_bg")) {
            this.load.image("laut_bg", "Asset/Laut/bglaut.png");
        }
        if (!this.textures.exists("laut_timer")) {
            this.load.image("laut_timer", "Asset/Laut/Timer.png");
        }
        if (!this.textures.exists("laut_gelembung")) {
            this.load.image("laut_gelembung", "Asset/Laut/Gelembung.png");
        }
        if (!this.textures.exists("laut_keranjang")) {
            this.load.image("laut_keranjang", "Asset/Laut/keranjang.png");
        }
        ...
        if (!this.cache.audio.exists("bgmLaut")) {
            this.load.audio("bgmLaut", "Asset/Sound/bgmLaut.mp3");
        }
    }
    ...
    create() {
        ...
        if (this.cache.audio.exists("bgmLaut")) {
            this.bgmLaut = this.sound.add("bgmLaut", { loop: true, volume: 0.4 });
            if (!this.isMuted) this.bgmLaut.play();
        }

        this.cameras.main.fadeIn(400, 0, 0, 0);

        const cfg = LautGameScene.LEVEL_CONFIG[this.level];
    ...
    update(time, delta) {
        ...
        if (this.hudCorrect) this.hudCorrect.setText("✅ " + this.scoreCorrect + " / " + LautGameScene.LEVEL_CONFIG[this.level].target);
    ...
    _buildBackground(w, h) {
        if (this.textures.exists("laut_bg")) {
            ...
            this.add.rectangle(w / 2, h / 2, w, h, 0x0a2a4a);
        }
        ...
        if (this.textures.exists("laut_gelembung")) {
            const spawnBubble = () => {
                ...
                const bubble = this.add.image(bx, h + 20, "laut_gelembung")
                ...
                this.tweens.add({
                    targets: bubble,
                    y: -50,
                    x: bx + Phaser.Math.Between(-50, 50),
                    duration: Phaser.Math.Between(3000, 7000),
                    ease: "Sine.easeInOut",
                    onComplete: () => { if (bubble) bubble.destroy(); }
                });

                this.time.delayedCall(Phaser.Math.Between(400, 1500), spawnBubble);
            };
            
            // Spawn initial bubbles
            for (let i = 0; i < 6; i++) {
                this.time.delayedCall(Phaser.Math.Between(0, 3000), spawnBubble);
            }
        }

        this.add.text(12, h - 12, "🌊 LAUT  |  Level " + this.level, {
            fontSize: "15px", fontFamily: "Arial, sans-serif",
            color: "#ffffff", stroke: "#002244", strokeThickness: 3,
    ...
    _buildBasket(w, h) {
        ...
        this.basket = this.physics.add.image(w / 2, basketY, "laut_keranjang");
    ...
    _buildHUD(w, h) {
        ...
        const style = {
            ...
            color: "#ffffff", stroke: "#002244", strokeThickness: 4,
        };

        this.hudCorrect = this.add.text(12, 12, "✅ 0 / " + LautGameScene.LEVEL_CONFIG[this.level].target, style).setOrigin(0, 0).setDepth(10);
        this.hudWrong = this.add.text(12, 12 + 25 * ui, "❌ 0", style).setOrigin(0, 0).setDepth(10);

        // Timer — kanan atas
        this.timerBg = this.add.image(w - 15 * ui, 15 * ui, "laut_timer").setOrigin(1, 0).setDepth(9);
    ...
    spawnAnimal(w) {
        ...
        const pool = isCorrect ? LautGameScene.ANIMALS.correct : LautGameScene.ANIMALS.trap;
        ...
        const randomScale = Phaser.Math.FloatBetween(0.85, 1.15); // Sedikit variasi
    ...
    _endLevel() {
        ...
        const isWin = this.scoreCorrect >= LautGameScene.LEVEL_CONFIG[this.level].target;

        if (isWin) {
            const saved = parseInt(localStorage.getItem("laut_maxLevel") || "1");
            const newMax = Math.max(saved, this.level + 1);
            localStorage.setItem("laut_maxLevel", String(Math.min(newMax, 3)));
    ...
    _showResult(isWin) {
        ...
        if (isWin) {
            ...
            if (this.scoreCorrect >= LautGameScene.LEVEL_CONFIG[this.level].target + 5) stars = 3;
        }
        ...
        const STAR_SIZE = 0.30;        // Skala visual bintang kuning
        // =====================================
        ...
        btnDefs.push({
            key: "ui_home", action: () => {
                ...
                this.cameras.main.once("camerafadeoutcomplete", () => { this.scene.start("LautScene"); });
            }
        });
        ...
        btnDefs.push({
            key: "ui_retry", action: () => {
                this.cameras.main.fadeOut(300, 0, 0, 0);
                this.cameras.main.once("camerafadeoutcomplete", () => { this.scene.start("LautGameScene", { level: this.level }); });
            }
        });
        ...
        if (isWinAndHasNext) {
            btnDefs.push({
                key: "ui_next", action: () => {
                    this.cameras.main.fadeOut(300, 0, 0, 0);
                    this.cameras.main.once("camerafadeoutcomplete", () => { this.scene.start("LautGameScene", { level: this.level + 1 }); });
                }
            });
        }
...
window.LautGameScene = LautGameScene;
