// ============================================================
//  DanauGameScene.js — Gameplay Habitat Danau (Self-Contained)
//  Progres disimpan: localStorage key "danau_maxLevel"
// ============================================================

class DanauGameScene extends Phaser.Scene {
    constructor() {
        super("DanauGameScene");
    }

    static get ANIMALS() {
        return {
            correct: ["danau_angsa", "danau_ikanlele", "danau_ikannila", "danau_japati", "danau_bebek"],
            trap: [
    ...
    preload() {
        if (!this.textures.exists("danau_bg"))        this.load.image("danau_bg",        "Asset/Danau/bgDanau.png");
        if (!this.textures.exists("danau_timer"))     this.load.image("danau_timer",     "Asset/Danau/Timer.png");
        if (!this.textures.exists("danau_keranjang")) this.load.image("danau_keranjang", "Asset/Danau/keranjang.png");
        if (!this.textures.exists("danau_awan"))      this.load.image("danau_awan",      "Asset/Danau/Awan.png");
        ...
        if (!this.cache.audio.exists("bgmDanau"))      this.load.audio("bgmDanau",      "Asset/Sound/bgmDanau.mp3");
    }
    ...
    create() {
        ...
        if (this.cache.audio.exists("bgmDanau")) {
            this.bgmDanau = this.sound.add("bgmDanau", { loop: true, volume: 0.4 });
            if (!this.isMuted) this.bgmDanau.play();
        }

        this.cameras.main.fadeIn(400, 0, 0, 0);

        const cfg = DanauGameScene.LEVEL_CONFIG[this.level];
        this.scoreCorrect = 0;
        this.scoreWrong   = 0;
        ...
        this.timerEvent = this.time.addEvent({
            delay: 1000, callback: this._onTick, callbackScope: this, repeat: cfg.timer - 1,
        });
    ...
    update() {
        ...
        if (this.hudCorrect) this.hudCorrect.setText("✅ " + this.scoreCorrect + " / " + DanauGameScene.LEVEL_CONFIG[this.level].target);
        if (this.hudWrong)   this.hudWrong.setText("❌ " + this.scoreWrong);
    }

    _buildBackground(w, h) {
        if (this.textures.exists("danau_bg")) {
            this.add.image(w / 2, h / 2, "danau_bg").setDisplaySize(w, h);
        } else {
            this.add.rectangle(w / 2, h / 2, w, h, 0x0a1e3a);
        }
        this.add.rectangle(w / 2, h / 2, w, h, 0x001020, 0.20);

        const ui = Math.min(w / 900, h / 550);
        if (this.textures.exists("danau_awan")) {
            const sc = ui * 0.6;
            const c1 = this.add.image(-160, h * 0.09, "danau_awan").setScale(sc).setAlpha(0.55).setDepth(2);
            const c2 = this.add.image(w + 160, h * 0.18, "danau_awan").setScale(sc * 0.72).setAlpha(0.38).setDepth(2);
            this.tweens.add({ targets: c1, x: w + 180, duration: 32000, ease: "Linear", repeat: -1, onRepeat: () => { c1.x = -160; } });
            this.tweens.add({ targets: c2, x: -160,    duration: 44000, ease: "Linear", repeat: -1, onRepeat: () => { c2.x = w + 180; } });
        }

        this.add.text(12, h - 12, "🏞️ DANAU  |  Level " + this.level, {
            fontSize: "15px", fontFamily: "Arial, sans-serif",
            color: "#ffffff", stroke: "#001428", strokeThickness: 3,
        }).setOrigin(0, 1).setAlpha(0.75).setDepth(3);
    }

    _buildBasket(w, h) {
        const ui = Math.min(w / 900, h / 550);
        this.basket = this.physics.add.image(w / 2, h - 72 * ui, "danau_keranjang");
    ...
    _buildHUD(w, h) {
        const ui    = Math.min(w / 900, h / 550);
        const style = { fontSize: `${Math.floor(18 * ui)}px`, fontFamily: "Arial Black, Arial, sans-serif", color: "#ffffff", stroke: "#001428", strokeThickness: 4 };

        this.hudCorrect = this.add.text(12, 12, "✅ 0 / " + DanauGameScene.LEVEL_CONFIG[this.level].target, style).setOrigin(0, 0).setDepth(10);
        this.hudWrong   = this.add.text(12, 12 + 25 * ui, "❌ 0", style).setOrigin(0, 0).setDepth(10);

        this.timerBg = this.add.image(w - 15 * ui, 15 * ui, "danau_timer").setOrigin(1, 0).setDepth(9);
    ...
    spawnAnimal(w) {
        ...
        const pool      = isCorrect ? DanauGameScene.ANIMALS.correct : DanauGameScene.ANIMALS.trap;
        ...
        animal.setScale(((DanauGameScene.SIZE_MAP[texKey] || 90) * ui / maxDim) * Phaser.Math.FloatBetween(0.88, 1.12));
    ...
    _onCatch(basket, animal) {
        if (animal.getData("isCorrect")) {
            this.scoreCorrect++;
            this._showFeedback(animal.x, animal.y, "+1", "#70ccff");
    ...
    _endLevel() {
        ...
        if (this.bgmDanau) this.bgmDanau.stop();
        this.animalGroup.getChildren().forEach(a => a.destroy());
        this.animalGroup.clear(true, true);

        const isWin = this.scoreCorrect >= DanauGameScene.LEVEL_CONFIG[this.level].target;
        if (isWin) {
            const saved = parseInt(localStorage.getItem("danau_maxLevel") || "1");
            localStorage.setItem("danau_maxLevel", String(Math.min(Math.max(saved, this.level + 1), 3)));
    ...
    _showResult(isWin) {
        ...
        const overlay = this.add.rectangle(w / 2, h / 2, w, h, 0x000d1a, 0.72).setDepth(30).setAlpha(0);
        ...
        const pH = panel.height * tps, pW = panel.width * tps;
        const pcX = panel.x, pcY = panel.y;

        let stars = 0;
        if (isWin) { stars = 1; if (this.scoreWrong <= 3) stars = 2; if (this.scoreWrong === 0) stars = 3; if (this.scoreCorrect >= DanauGameScene.LEVEL_CONFIG[this.level].target + 5) stars = 3; }
        ...
        this.add.text(pcX, pcY - pH * 0.56, isWin ? "LUAR BIASA!" : "AYO COBA LAGI!", { fontSize: `${Math.floor(pH * 0.08)}px`, fontFamily: "Arial Black, Arial, sans-serif", color: isWin ? "#70ccff" : "#ff8080", stroke: "#000", strokeThickness: 5 }).setOrigin(0.5).setDepth(32).setAlpha(0);
        ...
        const btnDefs = [
            { key: "ui_home",  action: () => { this.cameras.main.fadeOut(300, 0, 0, 0); this.cameras.main.once("camerafadeoutcomplete", () => { this.scene.start("DanauScene"); }); } },
            { key: "ui_retry", action: () => { this.cameras.main.fadeOut(300, 0, 0, 0); this.cameras.main.once("camerafadeoutcomplete", () => { this.scene.start("DanauGameScene", { level: this.level }); }); } },
        ];
        if (isWin && this.level < 3) btnDefs.push({ key: "ui_next", action: () => { this.cameras.main.fadeOut(300, 0, 0, 0); this.cameras.main.once("camerafadeoutcomplete", () => { this.scene.start("DanauGameScene", { level: this.level + 1 }); }); } });
...
window.DanauGameScene = DanauGameScene;
