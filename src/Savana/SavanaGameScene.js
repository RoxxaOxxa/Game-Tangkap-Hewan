// ============================================================
//  SavanaGameScene.js — Gameplay Habitat Savana (Self-Contained)
//  Progres disimpan: localStorage key "savana_maxLevel"
// ============================================================

class SavanaGameScene extends Phaser.Scene {
    constructor() {
        super("SavanaGameScene");
    }

    static get ANIMALS() {
        return {
            correct: ["savana_gajah", "savana_jerapah", "savana_singa", "savana_zebra", "savana_hyena"],
            trap: [
                ...
                "gunung_condor", "gunung_domba", "gunung_elang", "gunung_macan", "gunung_marmot",
                "rawa_buaya", "rawa_bangau", "rawa_ikangabus", "rawa_katak", "rawa_ularair",
            ],
        };
    }
    ...
    static get LEVEL_CONFIG() {
        return [
            null,
            { speed: 130, spawnMs: 1300, pctCorrect: 0.68, target: 8,  timer: 60 },
            { speed: 190, spawnMs: 950,  pctCorrect: 0.52, target: 10, timer: 45 },
            { speed: 260, spawnMs: 700,  pctCorrect: 0.42, target: 12, timer: 30 },
        ];
    }
    ...
    static get SIZE_MAP() {
        return {
            ...
            // Trap — Gunung
            "gunung_condor": 130, "gunung_domba": 100, "gunung_elang": 120, "gunung_macan": 140, "gunung_marmot": 70,
            // Trap — Rawa
            "rawa_buaya": 150, "rawa_bangau": 100, "rawa_ikangabus": 90, "rawa_katak": 60, "rawa_ularair": 120,
        };
    }
    ...
    preload() {
        if (!this.textures.exists("savana_bg"))        this.load.image("savana_bg",        "Asset/Savana/bgSavana.png");
        if (!this.textures.exists("savana_timer"))     this.load.image("savana_timer",     "Asset/Savana/Timer.png");
        if (!this.textures.exists("savana_keranjang")) this.load.image("savana_keranjang", "Asset/Savana/keranjang.png");
        if (!this.textures.exists("savana_awan"))      this.load.image("savana_awan",      "Asset/Savana/Awan.png");

        // Hewan Savana (benar)
        this.load.image("savana_gajah",   "Asset/Hewan/HewanSavana/Gajah.png");
        this.load.image("savana_jerapah", "Asset/Hewan/HewanSavana/Jerapah.png");
        this.load.image("savana_singa",   "Asset/Hewan/HewanSavana/Singa.png");
        this.load.image("savana_zebra",   "Asset/Hewan/HewanSavana/Zebra.png");
        this.load.image("savana_hyena",   "Asset/Hewan/HewanSavana/hyena.png");
        ...
        // Trap — Rawa
        this.load.image("rawa_buaya",    "Asset/Hewan/HewanRawa/Buaya.png");
        this.load.image("rawa_bangau",   "Asset/Hewan/HewanRawa/bangau.png");
        this.load.image("rawa_ikangabus","Asset/Hewan/HewanRawa/ikanGabus.png");
        this.load.image("rawa_katak",    "Asset/Hewan/HewanRawa/katak.png");
        this.load.image("rawa_ularair",  "Asset/Hewan/HewanRawa/ularAir.png");
        ...
        if (!this.cache.audio.exists("bgmSavana"))     this.load.audio("bgmSavana",     "Asset/Sound/bgmSavana.mp3");
    }
    ...
    create() {
        ...
        if (this.cache.audio.exists("bgmSavana")) {
            this.bgmSavana = this.sound.add("bgmSavana", { loop: true, volume: 0.4 });
            if (!this.isMuted) this.bgmSavana.play();
        }

        this.cameras.main.fadeIn(400, 0, 0, 0);

        const cfg = SavanaGameScene.LEVEL_CONFIG[this.level];
        this.scoreCorrect = 0;
        this.scoreWrong   = 0;
    ...
    update() {
        ...
        if (this.hudCorrect) this.hudCorrect.setText("✅ " + this.scoreCorrect + " / " + SavanaGameScene.LEVEL_CONFIG[this.level].target);
        if (this.hudWrong)   this.hudWrong.setText("❌ " + this.scoreWrong);
    }

    _buildBackground(w, h) {
        if (this.textures.exists("savana_bg")) {
            this.add.image(w / 2, h / 2, "savana_bg").setDisplaySize(w, h);
        } else {
            this.add.rectangle(w / 2, h / 2, w, h, 0x3a2200);
        }
        this.add.rectangle(w / 2, h / 2, w, h, 0x100800, 0.20);

        const ui = Math.min(w / 900, h / 550);
        if (this.textures.exists("savana_awan")) {
            const sc = ui * 0.6;
            const c1 = this.add.image(-160, h * 0.08, "savana_awan").setScale(sc).setAlpha(0.55).setDepth(2).setTint(0xffd890);
            const c2 = this.add.image(w + 160, h * 0.17, "savana_awan").setScale(sc * 0.72).setAlpha(0.38).setDepth(2).setTint(0xffc060);
            this.tweens.add({ targets: c1, x: w + 180, duration: 30000, ease: "Linear", repeat: -1, onRepeat: () => { c1.x = -160; } });
            this.tweens.add({ targets: c2, x: -160,    duration: 44000, ease: "Linear", repeat: -1, onRepeat: () => { c2.x = w + 180; } });
        }

        this.add.text(12, h - 12, "🦁 SAVANA  |  Level " + this.level, {
            fontSize: "15px", fontFamily: "Arial, sans-serif",
            color: "#ffffff", stroke: "#1a0e00", strokeThickness: 3,
        }).setOrigin(0, 1).setAlpha(0.75).setDepth(3);
    }

    _buildBasket(w, h) {
        const ui = Math.min(w / 900, h / 550);
        this.basket = this.physics.add.image(w / 2, h - 72 * ui, "savana_keranjang");
    ...
    _buildHUD(w, h) {
        const ui    = Math.min(w / 900, h / 550);
        const style = { fontSize: `${Math.floor(18 * ui)}px`, fontFamily: "Arial Black, Arial, sans-serif", color: "#ffffff", stroke: "#1a0e00", strokeThickness: 4 };
        this.hudCorrect = this.add.text(12, 12, "✅ 0 / " + SavanaGameScene.LEVEL_CONFIG[this.level].target, style).setOrigin(0, 0).setDepth(10);
        this.hudWrong   = this.add.text(12, 12 + 25 * ui, "❌ 0", style).setOrigin(0, 0).setDepth(10);

        this.timerBg = this.add.image(w - 15 * ui, 15 * ui, "savana_timer").setOrigin(1, 0).setDepth(9);
    ...
    spawnAnimal(w) {
        ...
        const pool      = isCorrect ? SavanaGameScene.ANIMALS.correct : SavanaGameScene.ANIMALS.trap;
        ...
        animal.setScale(((SavanaGameScene.SIZE_MAP[texKey] || 90) * ui / maxDim) * Phaser.Math.FloatBetween(0.88, 1.12));
        ...
        this.tweens.add({ targets: animal, angle: 13 * sign, duration: Phaser.Math.Between(400, 650), yoyo: true, repeat: -1, ease: "Sine.easeInOut" });
    }

    _onCatch(basket, animal) {
        if (animal.getData("isCorrect")) {
            this.scoreCorrect++;
            this._showFeedback(animal.x, animal.y, "+1", "#ffd080");
    ...
    _endLevel() {
        ...
        if (this.bgmSavana) this.bgmSavana.stop();
        this.animalGroup.getChildren().forEach(a => a.destroy());
        this.animalGroup.clear(true, true);

        const isWin = this.scoreCorrect >= SavanaGameScene.LEVEL_CONFIG[this.level].target;
        if (isWin) {
            const saved = parseInt(localStorage.getItem("savana_maxLevel") || "1");
            localStorage.setItem("savana_maxLevel", String(Math.min(Math.max(saved, this.level + 1), 3)));
    ...
    _showResult(isWin) {
        ...
        const overlay = this.add.rectangle(w / 2, h / 2, w, h, 0x0a0400, 0.72).setDepth(30).setAlpha(0);
        ...
        let stars = 0;
        if (isWin) { stars = 1; if (this.scoreWrong <= 3) stars = 2; if (this.scoreWrong === 0) stars = 3; if (this.scoreCorrect >= SavanaGameScene.LEVEL_CONFIG[this.level].target + 5) stars = 3; }
        ...
        this.add.text(pcX, pcY - pH * 0.56, isWin ? "LUAR BIASA!" : "AYO COBA LAGI!", { fontSize: `${Math.floor(pH * 0.08)}px`, fontFamily: "Arial Black, Arial, sans-serif", color: isWin ? "#ffd080" : "#ff8080", stroke: "#000", strokeThickness: 5 }).setOrigin(0.5).setDepth(32).setAlpha(0);
        ...
        const btnDefs = [
            { key: "ui_home",  action: () => { this.cameras.main.fadeOut(300, 0, 0, 0); this.cameras.main.once("camerafadeoutcomplete", () => { this.scene.start("SavanaScene"); }); } },
            { key: "ui_retry", action: () => { this.cameras.main.fadeOut(300, 0, 0, 0); this.cameras.main.once("camerafadeoutcomplete", () => { this.scene.start("SavanaGameScene", { level: this.level }); }); } },
        ];
        if (isWin && this.level < 3) btnDefs.push({ key: "ui_next", action: () => { this.cameras.main.fadeOut(300, 0, 0, 0); this.cameras.main.once("camerafadeoutcomplete", () => { this.scene.start("SavanaGameScene", { level: this.level + 1 }); }); } });
...
window.SavanaGameScene = SavanaGameScene;
