// ============================================================
//  GunungScene.js — Pilih Level Gunung (dengan Level Lock)
//  Progres disimpan: localStorage key "gunung_maxLevel"
// ============================================================

class GunungScene extends Phaser.Scene {
    constructor() {
        super("GunungScene");
        this.isMuted = false;
    }
    ...
    preload() {
        this.load.image("gunung_bg",     "Asset/Gunung/bgGunung.png");
        this.load.image("gunung_lv1",    "Asset/Gunung/1.png");
        this.load.image("gunung_lv2",    "Asset/Gunung/2.png");
        this.load.image("gunung_lv3",    "Asset/Gunung/3.png");
        this.load.image("gunung_gembok", "Asset/Gunung/gembok.png");
        this.load.image("gunung_awan",   "Asset/Gunung/Awan.png");
    ...
    _getMaxLevel() {
        return parseInt(localStorage.getItem("gunung_maxLevel") || "1");
    }
    ...
    _buildLayout() {
        ...
        this.add.rectangle(w / 2, h / 2, w, h, 0x080e18, 0.40);

        // ── Awan animasi ──
        const cloudSc = ui * 0.6;
        const cloud1 = this.add.image(-160, h * 0.08, "gunung_awan").setScale(cloudSc).setAlpha(0.80).setDepth(1);
        const cloud2 = this.add.image(w + 160, h * 0.17, "gunung_awan").setScale(cloudSc * 0.75).setAlpha(0.60).setDepth(1);
        ...
        // ── Judul ──
        const title = this.add.text(w / 2, -60, "⛰️  HABITAT GUNUNG", {
            fontSize: `${Math.floor(48 * ui)}px`,
            fontFamily: "Arial Black, Arial, sans-serif",
            color: "#d8eeff", stroke: "#08101c", strokeThickness: 6,
        ...
        const sub = this.add.text(w / 2, -60, "Pilih Level", {
            ...
            color: "#aaccee", stroke: "#08101c", strokeThickness: 3,
        }).setOrigin(0.5).setAlpha(0).setDepth(2);
        ...
        // ── Tombol Level ──
        const levels = [
            { key: "gunung_lv1", level: 1, desc: "Mudah" },
            { key: "gunung_lv2", level: 2, desc: "Sedang" },
            { key: "gunung_lv3", level: 3, desc: "Sulit" },
        ];
        ...
        levels.forEach((lvl, i) => {
            ...
            if (!unlocked) btn.setTint(0x505560);
                    ...
                    if (unlocked) {
                        this._makeBouncyButton(btn, btnScale, () => {
                            ...
                            this.cameras.main.once("camerafadeoutcomplete", () => {
                                this.scene.start("GunungGameScene", { level: lvl.level });
                            });
                        });
                    } else {
            ...
            if (!unlocked) {
                const lock = this.add.image(bx, btnY, "gunung_gembok").setOrigin(0.5).setAlpha(0).setDepth(4);
            ...
            const descTxt = this.add.text(bx, btnY + 80 * ui, lvl.desc, {
                ...
                fontFamily: "Arial, sans-serif",
                color: unlocked ? "#aaccee" : "#505560",
                stroke: "#08101c", strokeThickness: 3,
        ...
        backTxt.on("pointerout",  () => { this.tweens.add({ targets: backTxt, scale: 1,    duration: 120, ease: "Back.out(2)" }); backTxt.setStyle({ color: "#aaccee" }); });
...
window.GunungScene = GunungScene;
